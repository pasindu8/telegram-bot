...
# (All conversation handlers like start_send_message, get_yt_url etc. remain the same)
...
async def get_download_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text('මෙම ක්‍රියාවලිය තවම සකසා නැත.')
    return ConversationHandler.END
...
async def unhandled_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("මට ඔබව තේරුම් ගත නොහැක. /start භාවිතා කරන්න.")
...
def create_bot_instance():
    ...
    conv_handler_send_msg = ConversationHandler(
    ...
    # Add other handlers...
    app.add_handler(conv_handler_send_msg)
    app.add_handler(conv_handler_yt)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, unhandled_message))
    
    return app

# Global bot instance, created once
bot_instance = create_bot_instance()

# --- Vercel Handler ---
async def process_update_async(body: Dict[str, Any]):
    """Asynchronously processes a single update from Telegram."""
    if not bot_instance:
        logger.error("Bot instance is not initialized.")
        return
    
    update = Update.de_json(body, bot_instance.bot)
...
def handler(request):
    ...
    try:
        ...
        if request.method == 'POST':
            asyncio.run(process_update_async(json.loads(request.body.decode('utf-8'))))
